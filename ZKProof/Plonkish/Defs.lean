...
/-- Check a fixed constraint. -/
lemma check_fixed {x : C.Instance} (sat : Satisfying C.R x) (e : C.FixedEntry)
    : sat.w e = C.f e := by
  obtain ⟨ fixed ⟩ := sat.satisfied
  rw [fixed e]

/-- Check a fixed constraint. -/
lemma check_input {x : C.Instance} (sat : Satisfying C.R x) (k : Fin C.t)
    : x[k] = sat.w C.S[k] := by
  obtain ⟨ _, input ⟩ := sat.satisfied
  rw [input k]

/-- Check an equality constraint. -/
lemma check_equal {x : C.Instance} (sat : Satisfying C.R x) (e e' : C.Entry)
    (equiv : C.E e e') : sat.w e = sat.w e' := by
...
by
  calc x = sat.w dt_a := (dt F).check_input F sat (0 : Fin 1)
       _ = sat.w dt_c := (dt F).check_equal F sat dt_a dt_c rfl
       _ = 42         := (dt F).check_fixed F sat ⟨dt_c, by simp [dt, dt_c, dt_entry, entry]⟩
  ...
  fun (x' : Vector F 1) (sat' : Satisfying (dt F).R x') => {
    ...
    satisfied := by
      ...
      calc x = x'[0]       := by simp_all only [x]; rfl
           _ = sat'.w dt_a := (dt F).check_input F sat' (0 : Fin 1)
           _ = sat'.w dt_c := (dt F).check_equal F sat' dt_a dt_c rfl
           _ = 42          := (dt F).check_fixed F sat' ⟨dt_c, by simp [dt, dt_c, dt_entry, entry]⟩
  }
